            ...
            if in_pop:
                ...
                if stripped.startswith('('):
                    ...
                    if len(vals) >= 3:
                        county = vals[1].lower()
                        year = str(vals[2])
                        if county == 'augusta' and year == '1870':
                            augusta_1870_rows.append(vals)

    print(f"Writing {len(augusta_1870_rows)} rows to {output_csv}...")
    with open(output_csv, 'w', encoding='utf-8', newline='') as out_f:
        writer = csv.writer(out_f)
        writer.writerow(columns)
        writer.writerows(augusta_1870_rows)

    print(f"Successfully generated {output_csv} with {len(augusta_1870_rows)} rows and {len(columns)} columns.")
